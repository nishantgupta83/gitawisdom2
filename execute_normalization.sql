-- EXECUTE Database Normalization - Final SQL
-- PostgreSQL ARRAY syntax with real scenario IDs
-- Only categories with underscores from actual database

-- Execute this file to normalize scenario_translations table:
-- psql -d your_database -f execute_normalization.sql
...
-- Marriage Expectations (postponed_marriage_expectations -> marriage expectations)
UPDATE scenario_translations 
...
-- Body Confidence (self_image_body_confidence -> body confidence)
UPDATE scenario_translations 
...
-- SELECT category, COUNT(*) as count FROM scenario_translations 
-- WHERE category LIKE '%_%' GROUP BY category ORDER BY count DESC;